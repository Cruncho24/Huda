...
const state = {
  ...
  verseIndex: (() => { const len = typeof VERSES_OF_DAY !== 'undefined' ? VERSES_OF_DAY.length : 1; return Math.floor(Math.random() * len); })(),
  ...
  streak: (() => { try { const r = JSON.parse(localStorage.getItem('huda_streak') || '{}'); return { count: parseInt(r.count, 10) || 0, lastDate: typeof r.lastDate === 'string' ? r.lastDate : null }; } catch(e) { return { count: 0, lastDate: null }; } })(),
...
// minimal=true: just Arabic + English, vertically centred, no branding.
...
async function showShareCardModal(opts) {
  ...
  const canFileShare = !!(navigator.canShare?.({ files: [new File([], 'x.png', { type: 'image/png' })] }));
